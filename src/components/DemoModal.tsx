...
export function DemoModalProvider({ children }: { children: ReactNode }) {
  ...
  const waMsg = encodeURIComponent(t("demo.waMessage").replace("{action}", action));
      ...
      {open && (
        <div className="fixed inset-0 z-[200] flex items-center justify-center p-4">
          ...
          >
            ...
            {/* Content */}
            <h3 className="text-lg font-bold text-ink text-center">{t("demo.title")}</h3>
            <p className="text-sm text-ink-muted text-center mt-2 leading-relaxed">
              <span className="font-semibold text-celeste-dark">{action}</span> {t("demo.desc")}
            </p>
            <p className="text-xs text-ink-muted text-center mt-1">{t("demo.contactCta")}</p>
