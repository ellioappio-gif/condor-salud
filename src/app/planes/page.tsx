...
function TierCard({
  ...
  const inheritedLabel =
    presetIdx > 0 ? `${t("planes.everythingIn")} ${PRESETS[presetIdx - 1]?.name ?? ""}` : null;
    ...
    >
      ...
      <div className="mb-5">
        {hasDiscount && (
          <div className="flex items-center gap-2 mb-1">
            ...
            <span className="px-1.5 py-0.5 text-[10px] font-bold bg-green-100 text-green-700 rounded">
              {Math.round(preset.discount * 100)}% {t("planes.discount")}
            </span>
          </div>
        )}
        <div className="flex items-baseline gap-1">
          ...
          <span className="text-sm text-ink-muted">{t("planes.perMonth")}</span>
        </div>
      </div>
      ...
      <div className="flex-1 space-y-2 mb-6">
        {inheritedLabel && (
          <p className="text-xs font-semibold text-celeste-dark mb-2">
            {inheritedLabel}
            {t("planes.plusMore")}
          </p>
        )}
      ...
      <div className="mt-auto space-y-2">
        ...
        >
          {active ? t("planes.selectedPlan") : t("planes.choosePlan")}
        </button>
        <p className="text-center text-[10px] text-ink-muted">
          {preset.modules.length} {t("planes.modulesIncluded")}
          {preset.annual ? ` · ${t("planes.annualAvailable")}` : ""}
        </p>
      </div>
    </div>
  );
                ...
                <div className="flex-1 min-w-0">
                  ...
                  {mod.deps && mod.deps.length > 0 && (
                    <p className="text-[10px] text-celeste-dark mt-0.5">
                      {t("planes.requires")} {mod.deps.map((d) => getModule(d).label).join(", ")}
                    </p>
                  )}
                </div>
                <div className="text-right shrink-0">
                  <span className="text-sm font-semibold text-ink">{formatARS(mod.price)}</span>
                  <span className="text-[10px] text-ink-muted block">{t("planes.perMonth")}</span>
                </div>
              </label>
            );
          })}
...
const FAQ_KEYS = [
  { q: "planes.faq1q", a: "planes.faq1a" },
  { q: "planes.faq2q", a: "planes.faq2a" },
  { q: "planes.faq3q", a: "planes.faq3a" },
  { q: "planes.faq4q", a: "planes.faq4a" },
  { q: "planes.faq5q", a: "planes.faq5a" },
  { q: "planes.faq6q", a: "planes.faq6a" },
];
  ...
  return (
    <div className="min-h-screen bg-white">
      ...
      <div id="custom" ref={customRef} className="max-w-[960px] mx-auto px-6 py-2">
        <div className="flex items-center gap-4">
          ...
          <span className="text-xs font-semibold text-ink-muted whitespace-nowrap flex items-center gap-2">
            <Zap className="w-3.5 h-3.5 text-celeste" />
            {t("planes.orCustom")}
          </span>
          <div className="flex-1 h-px bg-border" />
        </div>
      ...
      <section className="max-w-[960px] mx-auto px-6 py-10">
        <div className="lg:grid lg:grid-cols-[1fr_320px] lg:gap-8">
          ...
          <div className="space-y-4">
            <div className="mb-6">
              <h2 className="text-xl font-bold text-ink">{t("planes.customPlan")}</h2>
              <p className="text-sm text-ink-muted mt-1">{t("planes.customPlanDesc")}</p>
            </div>
            {CATEGORIES.map((cat) => (
              <CategoryAccordion
                key={cat.id}
          ...
          <div className="hidden lg:block">
            ...
            >
              <h3 className="text-sm font-bold text-ink mb-4">{t("planes.summary")}</h3>

              {activePresetDef && (
                <div className="mb-4 px-3 py-2 bg-celeste-pale rounded-lg">
                  <p className="text-xs font-semibold text-celeste-dark">
                    Plan {activePresetDef.name} {t("planes.activePlanLabel")}
                  </p>
                </div>
              )}
              ...
              <div className="space-y-2 text-sm">
                <div className="flex justify-between">
                  <span className="text-ink-muted">{t("planes.modules")}</span>
                  <span className="font-semibold text-ink">{plan.selectedModules.length}</span>
                </div>
                <div className="flex justify-between">
                  <span className="text-ink-muted">{t("planes.subtotal")}</span>
                ...
                {discountAmount > 0 && (
                  <div className="flex justify-between text-green-600">
                    <span>
                      {t("planes.discountLabel")} ({Math.round(plan.discount * 100)}%)
                    </span>
                    <span>-{formatARS(discountAmount)}</span>
                  </div>
                )}
                <div className="border-t border-border pt-2 flex justify-between">
                  <span className="font-bold text-ink">{t("planes.total")}</span>
              ...
              <div className="mt-6 space-y-2">
                ...
                <a
                  href={whatsappUrl(t("planes.whatsappMsg"))}
                  target="_blank"
        ...
        <div className="flex items-center justify-between gap-3">
          <div>
            <p className="text-xs text-ink-muted">
              {plan.selectedModules.length} {t("planes.modules").toLowerCase()}
      ...
      <section className="max-w-[640px] mx-auto px-6 py-16 pb-32 lg:pb-16">
        <h2 className="text-xl font-bold text-ink mb-6 text-center">{t("planes.faqTitle")}</h2>
        <div className="border-t border-border">
          {FAQ_KEYS.map((item) => (
            <FAQItem key={item.q} q={t(item.q)} a={t(item.a)} />
          ))}
        </div>
      </section>
      ...
      <footer className="border-t border-border px-6 py-8 text-center">
        <p className="text-xs text-ink-muted">{t("planes.footerText")}</p>
      </footer>
    </div>
  );
}
