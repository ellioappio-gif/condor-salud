    ...
    <div className="space-y-5">
      ...
      <div className="bg-white border-2 border-celeste rounded-lg p-6">
        <div className="flex flex-wrap items-start justify-between gap-4">
          <div>
            <span className="px-2.5 py-1 text-[10px] font-bold tracking-widest uppercase bg-celeste-pale text-celeste-dark rounded">Plan Actual</span>
            <h2 className="text-2xl font-bold text-ink mt-3">Pro</h2>
            <p className="text-sm text-ink-muted mt-1">Ideal para clínicas de 3-10 profesionales. Incluye auditoría automática, integraciones con financiadores y soporte prioritario.</p>
      ...
      <div className="bg-white border border-border rounded-lg p-5">
        <h3 className="text-xs font-bold tracking-wider text-ink-muted uppercase mb-4">Uso del Plan</h3>
          ...
          {[
            ...
            <div key={u.label}>
              <div className="flex justify-between text-xs mb-1.5">
                ...
                <span className="font-bold text-ink">{u.used}{u.unit} / {u.limit}{u.unit}</span>
              </div>
              <div className="h-2 bg-border-light rounded-full overflow-hidden">
                <div className="h-full rounded-full transition-all" style={{ width: `${(u.used / u.limit) * 100}%`, backgroundColor: (u.used / u.limit) > 0.8 ? "#ef4444" : (u.used / u.limit) > 0.5 ? "#F6B40E" : "#4A7FAF" }} />
              </div>
            </div>
          ))}
        </div>
      ...
      <div className="bg-white border border-border rounded-lg overflow-hidden">
        <div className="px-5 py-4 border-b border-border">
          <h3 className="text-xs font-bold tracking-wider text-ink-muted uppercase">Comparar Planes</h3>
        </div>
        <table className="w-full text-sm">
          <thead>
            <tr className="bg-[#F8FAFB] text-[10px] font-bold tracking-wider text-ink-muted uppercase">
              <th className="text-left px-5 py-2.5">Característica</th>
              <th className="text-center px-5 py-2.5">Starter<br /><span className="text-celeste-dark font-normal normal-case">$25K/mes</span></th>
              <th className="text-center px-5 py-2.5 bg-celeste-pale/30">Pro<br /><span className="text-celeste-dark font-normal normal-case">$75K/mes</span></th>
              <th className="text-center px-5 py-2.5">Enterprise<br /><span className="text-celeste-dark font-normal normal-case">$180K/mes</span></th>
            </tr>
          </thead>
          <tbody>
            {[
              ...
              <tr key={row.feat} className="border-t border-border-light">
                ...
                <td className="px-5 py-2.5 text-xs text-center font-semibold text-celeste-dark bg-celeste-pale/30">{row.p}</td>
      ...
      <div className="bg-white border border-border rounded-lg overflow-hidden">
        <div className="px-5 py-4 border-b border-border">
          <h3 className="text-xs font-bold tracking-wider text-ink-muted uppercase">Historial de Pagos</h3>
        </div>
        <table className="w-full text-sm">
          <thead>
            <tr className="bg-[#F8FAFB] text-[10px] font-bold tracking-wider text-ink-muted uppercase">
          ...
          <tbody>
            {[
              { fecha: "01/03/2026", concepto: "Plan Pro — Marzo 2026", monto: "$75.000", estado: "Pagado" },
              { fecha: "01/02/2026", concepto: "Plan Pro — Febrero 2026", monto: "$68.000", estado: "Pagado" },
              { fecha: "01/01/2026", concepto: "Plan Pro — Enero 2026", monto: "$68.000", estado: "Pagado" },
              { fecha: "01/12/2025", concepto: "Plan Pro — Diciembre 2025", monto: "$62.000", estado: "Pagado" },
              { fecha: "01/11/2025", concepto: "Plan Pro — Noviembre 2025", monto: "$62.000", estado: "Pagado" },
            ].map((p, i) => (
      ...
      {/* Payment method */}
      <div className="bg-white border border-border rounded-lg p-5">
        <h3 className="text-xs font-bold tracking-wider text-ink-muted uppercase mb-4">Método de Pago</h3>
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3">
            <div className="w-12 h-8 bg-ink rounded flex items-center justify-center text-white text-[9px] font-bold">VISA</div>
            <div>
      ...
      {/* Billing info */}
      <div className="bg-white border border-border rounded-lg p-5">
        <h3 className="text-xs font-bold tracking-wider text-ink-muted uppercase mb-4">Datos de Facturación</h3>
        <div className="grid sm:grid-cols-2 gap-4 text-sm">
          <div><span className="text-ink-muted text-xs">Razón social:</span> <span className="text-ink font-semibold text-xs"> Centro Médico San Martín SRL</span></div>
          <div><span className="text-ink-muted text-xs">CUIT:</span> <span className="text-ink font-mono text-xs"> 30-71234567-8</span></div>
